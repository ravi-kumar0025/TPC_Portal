        ...
        <div className="h-[calc(100vh-8rem)] flex gap-6 animate-[fade-in-up_0.4s_ease-out]">
            ...
            <div className="w-1/3 bg-white rounded-2xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-slate-100 flex flex-col overflow-hidden">
                <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                    <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Building2 className="text-blue-500" size={20} /> Verification Queue</h2>
                    <p className="text-xs font-semibold text-slate-500 mt-1">{pendingCompanies.length} pending requests</p>
                </div>
                    ...
                    {pendingCompanies.length === 0 ? (
                        ...
                        pendingCompanies.map(company => (
                            <div
                                ...
                                className={`p-4 rounded-xl cursor-pointer border transition-all ${selectedCompany?._id === company._id ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-slate-100 hover:border-blue-200 hover:bg-slate-50'}`}
                            >
                ...
                {selectedCompany ? (
                    <>
                        <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50/30">
                            <div>
                                <h2 className="text-2xl font-bold text-slate-900">{selectedCompany.companyName}</h2>
                                <p className="text-slate-500 font-medium mt-1 flex items-center gap-1"><Mail size={14} /> {selectedCompany.companyEmail}</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => handleVerification(selectedCompany._id, 'verified')} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-bold rounded-lg shadow-sm transition-colors flex items-center gap-2">
                                    <Check size={16} /> Approve
                                </button>
                                <button onClick={() => handleVerification(selectedCompany._id, 'rejected')} className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 text-sm font-bold rounded-lg transition-colors flex items-center gap-2">
                                    <X size={16} /> Discard
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="p-6 flex-1 overflow-y-auto">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="border border-slate-200 rounded-xl bg-slate-50 p-4">
                                    <p className="text-sm text-slate-600 mb-2"><span className="font-bold text-slate-800">Account ID:</span> {selectedCompany._id}</p>
                                    <p className="text-sm text-slate-600"><span className="font-bold text-slate-800">Login Email:</span> {selectedCompany.email}</p>
                                </div>
                            </div>
                        </div>
                    </>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                        ...
                        <p className="font-semibold text-lg">Select a company from the queue</p>
                        <p className="text-sm">Their verification details will appear here.</p>
                    </div>
                )}
            </div>
        </div>
